...
class StardogStrategy:
    ...
    def __init__(
        ...
        self.__admin = stardog.Admin(
            endpoint=triplestore_url, username=self.__uname, password=self.__pwd
        )
        ...
        try:
            ...
            print(
                "Unable to connect to Stardog instance: {}".format(
                    self.__connection_details
                )
            )
    ...
    def list_databases(cls, triplestore_url: str, **kwargs):
        ...
        __admin: stardog.Admin = stardog.Admin(
            endpoint=__stardog_endpoint, username=__uname, password=__pwd
        )
        databases = []
    ...
    def parse(self, source=None, location=None, data=None, format="turtle", **kwargs):
        ...
        if source is not None and isinstance(
            source, (io.IOBase, io.TextIOBase, io.BufferedIOBase, io.RawIOBase)
        ):
            content = source.read()
    ...
    def __convert_json_entrydict(self, entrydict: "Dict[str, str]") -> str:  # type: ignore
        if entrydict["type"] == "uri":
            if entrydict["value"].startswith(":"):
                return "<{}{}>".format(
                    self.__getBaseNamespace(), entrydict["value"][1:]
                )
            else:
